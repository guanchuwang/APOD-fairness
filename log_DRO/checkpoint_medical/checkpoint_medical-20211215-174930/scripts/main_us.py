...
class Agent:
    ...
    def BT(self, prob):
        sorted_proba = np.sort(prob, axis=1)
        return np.argsort(sorted_proba[:, -1] - sorted_proba[:, -2])[:1]

    def entropy(self, prob):
        predict_entropy = -(prob * np.log(prob)).sum(axis=1)
        predict_entropy[np.isnan(predict_entropy)] = 0.
        return np.argsort(predict_entropy)[-1:]
    ...
    def generate_sensitive_label(self):
        ...
        with torch.no_grad():
            ...
            unlabeled_index = [index for index in range(self.x_train.shape[0]) if (not self.if_label[index])]
            x = self.x_train[unlabeled_index]
            x_head = self.clf_body(x)
            y_prob = torch.softmax(self.clf_head(x_head), dim=1)

            label_index = self.entropy(y_prob)
