...
from utils import measure_objective_results_body_head_eop, estimate_objective_body_head_eop
...
class Test_agent:
    ...
    def validate(self, hyperparam):
        ...
        estimate_measures = estimate_objective_body_head_eop(self.x_val, self.y_val, z_prob_val, self.clf_body,
...
if __name__ == "__main__":
    ...
    save_checkpoint("../" + hyperparam["root_log_dir"] + "/test_measures_eop.pth.tar",
    ...
    save_checkpoint("../" + hyperparam["root_log_dir"] + "/vallina_measures_eop.pth.tar",
    ...
    mean_vallina_fpr0 = 1-np.array([[y['0_fnr'] for y in x] for x in vallina_measures_buffer]).mean(axis=0)
    mean_vallina_fpr1 = 1-np.array([[y['1_fnr'] for y in x] for x in vallina_measures_buffer]).mean(axis=0)
    mean_vallina_eop = mean_vallina_fpr0/mean_vallina_fpr1

    print("Mean Vallina result:")
    print("Acc", mean_vallina_accuracy)
    # print("Fpr", mean_vallina_fpr_diff)
    # print("Fnr", mean_vallina_fnr_diff)
    print("EO", mean_vallina_eop)
    ...
    mean_fpr0 = 1-np.array([[y['0_fnr'] for y in x] for x in choose_test_measures_buffer]).mean(axis=0)
    mean_fpr1 = 1-np.array([[y['1_fnr'] for y in x] for x in choose_test_measures_buffer]).mean(axis=0)
    mean_eop = mean_fpr0/mean_fpr1
