...
class Agent:
    ...
    def generate_sensitive_label(self):
        ...
        for index, instance_index in enumerate(local_index):
            ...
            # x_local = self.x_train[instance_index].unsqueeze(dim=0)
            # with torch.no_grad():
            #     x_local_head = self.clf_body(x_local)
            # y_prob = self.clf_head_tmp.predict_proba(x_local_head).squeeze(axis=0)
            # uncertainty = self.entropy(y_prob)
            #
            # obj = self.alpha * uncertainty + (1-self.alpha) * (-fairness_metric)
